...
class PolicyLossConfig(BaseConfig):
    ...
    loss_mode: str = "vanilla"  # Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg', 'sdpo'
...
# ============================================================================
# SDPO: Self-Distillation Configuration (from SDPO_original)
# ============================================================================
...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        alpha (float): Interpolation parameter for Jensen-Shannon divergence.
            - 0.0: Forward KL (student -> teacher)
            - 1.0: Reverse KL (teacher -> student)
            - (0, 1): Generalized JS divergence
        teacher_regularization (str): How to update teacher. Options: 'ema', 'trust-region'.
        teacher_update_rate (float): EMA update rate for teacher (when teacher_regularization='ema').
        full_logit_distillation (bool): Whether to use full vocabulary distribution.
        distillation_topk (Optional[int]): Number of top tokens to use for distillation.
        distillation_add_tail (bool): Whether to add tail probability for top-k distillation.
        is_clip (Optional[float]): Importance sampling clipping ratio.
        max_reprompt_len (int): Maximum length for reprompted teacher input.
        success_reward_threshold (float): Reward threshold to consider a sample successful.
        include_environment_feedback (bool): Whether to include environment feedback in teacher prompt.
        dont_reprompt_on_self_success (bool): Skip reprompting if current sample is successful.
        remove_thinking_from_demonstration (bool): Remove thinking trace from successful demonstrations.
        environment_feedback_only_without_solution (bool): Only use feedback when no solution available.
        solution_template (str): Template for formatting successful solutions.
        feedback_template (str): Template for formatting environment feedback.
    ...
    def __post_init__(self):
        ...
        valid_teacher_reg = ["ema", "trust-region"]
        if self.teacher_regularization not in valid_teacher_reg:
            raise ValueError(
                f"self_distillation.teacher_regularization must be one of {valid_teacher_reg}, "
                f"got {self.teacher_regularization}"
            )
