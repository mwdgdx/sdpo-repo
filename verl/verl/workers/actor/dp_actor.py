...
class TrustRegionTeacher(nn.Module):
    ...
    Used for trust-region style teacher regularization in SDPO.
    """
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(self, config: ActorConfig, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        self.teacher_module: nn.Module | None = None  # SDPO: For self-distillation
    ...
    # =========================================================================
    # SDPO: Teacher EMA update
    # =========================================================================
    def _update_teacher(self) -> None:
        """Update teacher model using EMA if SDPO is enabled."""
    ...
    def _forward_micro_batch(
        ...
        # SDPO: Use specified module or default to actor_module
        model = module if module is not None else self.actor_module
        
        # SDPO: Check if we need topk/all logps (requires non-fused kernels)
        ...
        if (return_all_logps or use_topk) and self.use_fused_kernels:
            raise ValueError("SDPO logit distillation requires disabling fused kernels (use_fused_kernels=False).")
    ...
    def update_policy(self, data: DataProto):
        ...
        # SDPO: Check if self-distillation is enabled
        ...
        if self_distillation_enabled:
            ...
            assert self_distillation_required_keys.issubset(set(data.batch.keys())), \
                f"SDPO requires keys: {self_distillation_required_keys - set(data.batch.keys())}"
        ...
        # SDPO does not use advantages, so skip selecting it to avoid KeyError
        ...
        did_update = False  # Track if at least one successful gradient update occurred
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # SDPO does not use advantages, use .get() to avoid KeyError
                    ...
                    # =========================================================
                    # SDPO: Self-Distillation Loss Branch
                    # =========================================================
                    if self_distillation_enabled:
        ...
        self.actor_optimizer.zero_grad()
        # SDPO: Update teacher model with EMA after successful policy update
        if did_update:
